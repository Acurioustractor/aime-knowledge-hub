#!/usr/bin/env python3
"""
Fix Missing Metadata for AIME Knowledge Hub Documents v2

This script specifically addresses missing:
- Summaries
- Themes (using the correct field name)
- Word Count
- Language
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('metadata_fix_v2.log'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger(__name__)

class MetadataFixerV2:
    def __init__(self):
...
"""
    ...
    def process_document(self, record: Dict[str, Any]):
        ...
        try:
            ...
            # Get text content - try Full Text first, then extract from PDF
            full_text = fields.get('Full Text', '').strip()
            
            # If no full text but has file attachment, try to extract
            if not full_text and 'File' in fields:
                files = fields['File']
                if files and len(files) > 0:
                    file_url = files[0]['url']
                    logger.info(f"  Extracting text from PDF: {file_url}")
                    full_text = self.extract_text_from_pdf_url(file_url)
                    
                    # If we extracted text, update the Full Text field
                    if full_text:
            ...
            needs_themes = not fields.get('Themes') or fields.get('Themes') == 'processed'
            needs_word_count = not fields.get('Word Count') or fields.get('Word Count') == 0
            needs_language = not fields.get('Language', '').strip()
            needs_full_text = not fields.get('Full Text', '').strip() and full_text
            
            updates = {}
            
            # Add full text if extracted
            if needs_full_text:
                updates['Full Text'] = full_text
                logger.info(f"  Added full text: {len(full_text)} characters")
            ...
            if needs_summary or needs_themes:
                if full_text:
                    ...
                    if needs_themes:
                        updates['Themes'] = ai_analysis['themes']
                        logger.info(f"  Themes: {ai_analysis['themes']}")
...
def main():
    """Main function."""
    logger.info("🎯 AIME Knowledge Hub - Metadata Fixer v2")
    logger.info("=========================================")
    
    try:
        fixer = MetadataFixerV2()
